...
void allocate(void)
{
    ...
    blocks_arrays = (double *****) ma_malloc(max_num_blocks*sizeof(double ****),
                                             __FILE__, __LINE__);

    for (n = 0; n < max_num_blocks; n++) {
        blocks[n].number = -1;
        blocks_arrays[n] = (double ****) ma_malloc(num_vars*sizeof(double ***),
                                                   __FILE__, __LINE__);
        ...
        for (m = 0; m < num_vars; m++) {
            ...
            //                                __FILE__, __LINE__);
            blocks_arrays[n][m] = (double ***) ma_malloc((x_block_size+2)*sizeof(double **),
                                                         __FILE__, __LINE__);
            for (i = 0; i < x_block_size+2; i++) {
                // blocks[n].array[m][i] = (double **)
                //                        ma_malloc((y_block_size+2)*sizeof(double *),
                //                                  __FILE__, __LINE__);
                blocks_arrays[n][m][i] = (double **)
                        ma_malloc((y_block_size+2)*sizeof(double *),
                                  __FILE__, __LINE__);
...
void deallocate(void)
{
    ...
    for (n = 0; n < max_num_blocks; n++) {
        for (m = 0; m < num_vars; m++) {
            for (i = 0; i < x_block_size+2; i++) {
                for (j = 0; j < y_block_size+2; j++)
                    free(blocks_arrays[n][m][i][j]);
                free(blocks_arrays[n][m][i]);
            }
            free(blocks_arrays[n][m]);
        }
        free(blocks_arrays[n]);
    }
    free(blocks_arrays);
    free(blocks);
